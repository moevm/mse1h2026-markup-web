...
class Dataset(Base):
    ...
    current_model_architecture: Mapped[str] = mapped_column(
        String(100), default="yolo11n"
    )
    pending_model_architecture: Mapped[Optional[str]] = mapped_column(
...
class PredictionBox(Base):
    ...
    batch_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("prediction_batch.id"), nullable=True
    )
    dataset_image_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("dataset_image.id"), nullable=True
    )
